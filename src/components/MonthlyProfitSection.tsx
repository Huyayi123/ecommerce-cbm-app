...
import type { AppProfile, MonthlyProfitDetail, MonthlyProfitSummary, SkuItem, TakealotSale } from '../types';
...
export function MonthlyProfitSection({ profile, skuItems, stores, summaries, onSave, onRefresh }: Props) {
  ...
  async function sync() {
    ...
    try {
      setSyncing(true); setDetails([]); setCurrentSummary(null);
      ...
      setCurrentSummary(result.summary); setDetails(result.details);
      await onRefresh();
  ...
  return <section className="monthly-profit-section">
    <div className="section-heading"><div><h2>老板月度利润</h2><p>按南非自然月统计销售、退货和广告费用；数据统一延迟 7 天结算。</p></div>
      <div className="export-actions"><button type="button" disabled={!displayed} onClick={() => displayed && exportMonthlyProfit(displayed, currentSummary === displayed ? details : [])}>导出 Excel</button><button className="primary" type="button" disabled={syncing} onClick={() => void sync()}>{syncing ? '同步中...' : '同步月度利润'}</button></div></div>
    <div className="profit-analysis-controls">
      <label>店铺<select value={shopName} onChange={(event) => { setShopName(event.target.value); setCurrentSummary(null); setDetails([]); }}>{stores.map((store) => <option key={store}>{store}</option>)}</select></label>
      <label>月份<input type="month" value={month} onChange={(event) => { setMonth(event.target.value); setCurrentSummary(null); setDetails([]); }} /></label>
    ...
    {details.length > 0 && <div className="table-wrap"><table><thead><tr><th>SKU</th><th>产品名称</th><th>销售数量</th><th>退货数量</th><th>销售利润</th><th>退货影响</th><th>净利润</th><th>异常原因</th></tr></thead><tbody>{details.map((row) => <tr key={row.sku}><td>{row.sku}</td><td>{row.productName || '-'}</td><td>{row.salesQuantity}</td><td>{row.returnQuantity}</td><td>{money(row.salesProfit)}</td><td>{money(row.returnProfitReversal + row.returnNetFees)}</td><td><strong>{money(row.netProfit)}</strong></td><td>{row.messages.join('；') || '正常'}</td></tr>)}</tbody></table></div>}
  </section>;
}
